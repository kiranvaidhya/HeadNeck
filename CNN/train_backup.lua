...
function train()
   ...
   for t = 1,trainData.size(),opt.batchSize do
      ...
      -- create mini batch
      -- local inputs = torch.Tensor(math.min(opt.batchSize,trainData:size()-t+1),1,21,21):cuda()
      ...
      local feval = function(x)
                       ...
                       -- estimate df/dW
                       local df_do = torch.Tensor(outputs:size(1), outputs:size(2)):cuda()
                       for i = 1,inputs:size(1) do
                          ...
                          f = f+err
                          -- -- estimate df/dW
                          -- local df_do = criterion:backward(outputs[i], targets[i])
