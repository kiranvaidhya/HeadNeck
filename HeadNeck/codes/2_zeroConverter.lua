...
truths = []
for i in xrange(23):
	patches.append(np.load(patch_root + 'zero_patches_' + str(i) + '.npy'))
	truths.append(np.load(patch_root + 'zero_truths_' + str(i) + '.npy'))
	]=])
else
	py.exec([=[
...
if validation == true then
	torch.save('tmp/zeroValidData.t7', torch_patches)
	torch.save('tmp/zeroValidLabel.t7', torch_truths)
else
	torch.save('tmp/zeroTrainData.t7', torch_patches)
	torch.save('tmp/zeroTrainLabel.t7', torch_truths)
end
